    ...
    for (const product of products) {
      ...
      if (!productCategory) {
        const newProductCategory: IProductCategory =
          await createProductCategoryByName(product.product_category as string);
...
export async function fetchProductByProductCategoryId(
  ...
  try {
    ...
    console.error(error);
    throw new Error("Error fetching products by product category id");
  }
}
